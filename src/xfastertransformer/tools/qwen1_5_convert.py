...
class Qwen1_5Convert(BaseModelConvert):
    """
    ...
    def split_and_convert(self, input_dir, output_dir, dtype, processes):
        ...
        for name, param in state_dict.items():
            ...
            # for merged weights, skip
                ...
                v_name = name.replace("q_proj", "v_proj")
                qkv = torch.cat(
                    (param.permute(0), state_dict[k_name].permute(0), state_dict[v_name].permute(0)), dim=0
                )
                model_named_parameters[
                    name.replace("self_attn.q_proj.bias", "attention.query_key_value.bias")
                ] = qkv
        ...
        for name, param in model_named_parameters.items():
            if name == "model.embed_tokens.weight": # "transformer.wte.weight":
            ...
            elif name == "model.norm.weight": #"transformer.ln_f.weight":
                ...
                if found :
                    ...
                    print(f"### not found layer {name} in {hf_model_name_pattern}")
        pool.close()
        pool.join()
